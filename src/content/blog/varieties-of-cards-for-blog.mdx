...
description: Learn how to use stunning card components including notebook-style, fancy animated cards, simple article cards, dogear bookmark cards, hover reveal cards, flip book cards, and circular image cards to create visually appealing content displays in your MDX blog posts with automatic dark mode support.
date: 2025-11-06
...
Looking for creative ways to showcase content in your blog posts? Our custom card components make it easy to present information in visually appealing, organized ways. We offer seven distinct card styles: **Notebook Cards** for a classic paper aesthetic, **Fancy Cards** for vibrant displays, **Simple Cards** for clean, professional article layouts, **Dogear Cards** for bookmarked highlights, **Hover Cards** for image galleries, **Book Cards** for interactive flip animations, and **Circle Cards** for circular profile pictures and icons. Let's explore all seven!
...
| Prop | Type | Required | Default | Description |
|------|------|----------|---------|-------------|
| `image` | string | Notebook: No<br/>Fancy: Yes<br/>Simple: Yes<br/>Dogear: N/A<br/>Hover: Yes<br/>Book: N/A<br/>Circle: No | - | URL or path to the image. Optional for Notebook/Circle cards. Not used in Dogear/Book cards. **Required for Hover cards**. For Circle cards, creates circular image display with fixed size (default) or natural size (with `isFreeSize={true}`). |
| `icon` | string | Book only | `'ri-book-open-line'` | Remix Icon class name (e.g., `ri-map-pin-line`) for Book cards |
| `title` | string | Most cards: Yes<br/>Circle: No | - | Card heading/title. For Circle cards, displays text in circle when no image provided. |
| `subtitle` | string | Book only | - | Subtitle text for Book cards |
| `imageAlt` | string | No | `title` value | Alt text for accessibility (not applicable to Dogear/Book cards). For Circle cards, defaults to title or 'Circle image'. |
| `heading` | string | Book only | - | Inside page heading for Book cards **(required for Book cards)** |
| `children` | ReactNode | Most cards: Yes<br/>Circle: N/A | - | Card content (paragraphs, HTML, etc.). Not used in Circle cards. |
...
| `size` | string | Circle only | `'auto'` | Size of the circle as CSS value (%, px, rem, etc.). When `'auto'`, uses `width` and `height` props. For responsive circles, use percentage values (e.g., `'20%'`). |
| `width` | string | Circle only | `'200px'` | Width of circle (used when `size='auto'`). Set equal to `height` for perfect circles. |
| `height` | string | Circle only | `'200px'` | Height of circle (used when `size='auto'`). Set equal to `width` for perfect circles. |
| `isFreeSize` | boolean | Circle only | `false` | When `true`, image displays at natural dimensions with circular/elliptical crop. Ignores `size`, `width`, and `height` props. |
...
| Feature | Notebook Cards | Fancy Cards | Simple Cards | Dogear Cards | Hover Cards | Book Cards | Circle Cards |
|---------|---------------|-------------|--------------|--------------|-----------------|-------------|--------------|
| **Style** | Classic notebook paper | Modern vibrant design | Clean professional | Bookmarked page | **Image reveal** | **Flip animation** | **Circular** |
| **Animations** | None | Card scale on hover | Image zoom + icon slide | None | **Slide-up reveal** | **Flip + expand** | **None** |
| **Colors** | Blue lines, red margin | Purple, aqua, violet | Theme-based minimal | 8 predefined + custom | **Theme-adaptive** | **4 gradients** | **Custom bg colors** |
| **Special Classes** | No | `upcharge`, `note` | No | No | **No** | **No** | **No** |
| **Typography** | Monospace | Georgia + Sans-serif | System fonts | Marvel | **System fonts** | **System fonts** | **System fonts** |
| **Best For** | Documentation, tutorials | Products, menus, promotions | Articles, blogs, news | Notes, quotes, highlights | **Galleries, wildlife, portfolios** | **Services, travel, products** | **Profiles, avatars, tech stacks** |
| **Hover Effect** | None | Card scale | Image zoom + arrow animation | None | **Description overlay** | **Flip + width expand** | **None** |
| **Read More Link** | No | No | Yes (optional) | No | **No** | **No** | **No** |
| **Images** | Optional | Required | Required | Text-only | **Required** | **No (uses icons)** | **Optional** |
| **Icons** | No | No | No | No | **No** | **Yes (Remix Icons)** | **No** |
| **Text Mode** | No | No | No | Text-only | **No** | **No** | **Yes** |
| **Shape** | Rectangle | Rectangle | Rectangle | Rectangle | **Rectangle** | **Rectangle** | **Circle** |
| **Client Hydration** | No | No | No | No | **Yes (required)** | **Yes (required)** | **No** |
...
1. **Choose the right card** - Notebook for documentation, Fancy for products, Simple for articles, Dogear for notes, Hover for image-heavy content, Book for services/offerings, Circle for profiles/avatars
...
16. **Client hydration** - Remember to add `client:load` to Hover and Book cards for interactivity
17. **Color consistency** - Book card gradient colors (city, ski, beach, camping) match their thematic content
18. **Dogear backgrounds** - Always set `bgColor` to match your page background for proper dogear effect
19. **Color meaning** - Use Dogear card colors meaningfully (red for alerts, green for success, etc.)
20. **Hover card images** - Use compelling images that work well with text overlays
