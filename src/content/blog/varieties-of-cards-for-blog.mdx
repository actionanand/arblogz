...
|------|------|----------|---------|-------------|
| `image` | string | Notebook: No<br/>Fancy: Yes<br/>Simple: Yes<br/>Dogear: N/A<br/> Hover: Yes | - | URL or path to the image. Optional for Notebook cards (text-only mode). Not used in Dogear cards. **Required for Hover cards** |
...
| `client:load` | directive | No <br/> Hover: Yes | - | **Required for Hover cards** to enable React hydration for interactivity. Not needed for other card types |
...
> **⚠️ Important Notes:**
> - **Hover Cards** require `client:load` directive for interactive hover functionality
...
| Feature | Notebook Cards | Fancy Cards | Simple Cards | Dogear Cards | Hover Cards |
|---------|---------------|-------------|--------------|--------------|-----------------|
...
| **Special Classes** | No | `upcharge`, `note` | No | No | **No** |
| **Typography** | Monospace | Georgia + Sans-serif | System fonts | Marvel | **System fonts** |
| **Best For** | Documentation, tutorials | Products, menus, promotions | Articles, blogs, news | Notes, quotes, highlights | **Galleries, wildlife, portfolios** |
| **Hover Effect** | None | Card scale | Image zoom + arrow animation | None | **Description overlay** |
| **Read More Link** | No | No | Yes (optional) | No | **No** |
| **Images** | Optional | Required | Required | Text-only | **Required** |
| **Client Hydration** | No | No | No | No | **Yes (required)** |
...
1. **Choose the right card** - Notebook for documentation, Fancy for products, Simple for articles, Dogear for notes, Hover for image-heavy content
