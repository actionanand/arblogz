...
export async function GET(context) {
  ...
  // Return raw RSS XML if format=xml is in the query
  if (format === 'xml') {
    return rss({
      title: site.title,
  ...
  // Otherwise, return user-friendly HTML page with instructions
  ...
  <div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-8 mb-6">
      <div class="flex items-center gap-4 mb-4">
        ${site.avatar ? `<img src="${site.avatar}" alt="${site.author}" class="w-16 h-16 rounded-full object-cover border-2 border-green-500" />` : ''}
        <div>
          ...
          ${site.author ? `<p class="text-sm text-gray-600">by ${site.author}</p>` : ''}
        </div>
      </div>
